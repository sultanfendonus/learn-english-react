...
class NewFeedbackContainer extends Component {
    ...
    submitForm(){
        if(this.state.message){
            this.props.newFeedback({user_name: this.state.name, category: this.state.category, message: this.state.message})
        ...
        return (
            <div>
                <Card title="New Feedback" style={{ }}>
                    ...
                    <Input value={this.state.name} onChange={(e)=>this.setState({name: e.target.value})} style={{marginBottom: '10px'}} placeholder="Name" />
                    ...
                    <TextArea value={this.state.message} style={{marginBottom: '15px'}} onChange={(e)=>this.setState({message: e.target.value})} rows={8} />
